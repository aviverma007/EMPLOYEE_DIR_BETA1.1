...
const MeetingRooms = () => {
  ...
  const handleBulkBookRoom = async () => {
    try {
      // Convert form data to API format
      const apiData = {
        employee_id: bulkBookingForm.employee_id,
        time_slots: bulkBookingForm.time_slots.map(slot => ({
          start_time: slot.start_time ? `${slot.start_time}:00Z` : "",
          end_time: slot.end_time ? `${slot.end_time}:00Z` : "",
          remarks: slot.remarks || ""
        })).filter(slot => slot.start_time && slot.end_time)
      };

      if (apiData.time_slots.length === 0) {
        alert('Please add at least one time slot');
        return;
      }

      const response = await fetch(`${backendUrl}/api/meeting-rooms/${selectedRoom.id}/book-multiple`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        setBulkBookingForm({
          employee_id: "",
          time_slots: [{ start_time: "", end_time: "", remarks: "" }]
        });
        setEmployeeSearch("");
        fetchRooms();
        alert(`Successfully booked ${apiData.time_slots.length} time slots!`);
      } else {
        const error = await response.json();
        alert(`Error booking rooms: ${error.detail}`);
      }
    } catch (error) {
      console.error('Error bulk booking room:', error);
      alert('Failed to book rooms');
    }
  };

  const addTimeSlot = () => {
    setBulkBookingForm(prev => ({
      ...prev,
      time_slots: [...prev.time_slots, { start_time: "", end_time: "", remarks: "" }]
    }));
  };
